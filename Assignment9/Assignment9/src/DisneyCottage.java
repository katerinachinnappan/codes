...
public class DisneyCottage extends JFrame implements ItemListener
{
	...
	private static final long serialVersionUID = 1L;
       ...
       /**Choices for user to pick**/
        JLabel dateChoice = new JLabel("<html>Please enter exact dates of your "
        		+ "stay. <br> (yyyy/MM/dd)</html>",  SwingConstants.CENTER);
        ...
        JLabel option = new JLabel("<html>Additional options: <br>"
        		+ "How long do you want to rent your boat?</html>");
        ...
        public void itemStateChanged(ItemEvent e)
        {
        	//create object
                ...
                //if user picks bed1 (600), calculate the total payment, disable bed2(85) and enable boat
                if(source == bed1)
                {
             	   diffDays = CalculationFrame.getDays(t1, t2);
                        ...
                        //if user diselects bed1(600), subtract the total payment and enable bed2(850)
                        else if(select == ItemEvent.DESELECTED)
                        {
                                price = price - 600.00 * (diffDays/7);
                                totalPrice.setText("Total Price: $" + price + "0 /week");
                ...
                //if user selectd bed2(850), calculate the total payment, disable bed1(600) and enable boat
                if(source == bed2)
                {
             	   diffDays = CalculationFrame.getDays(t1, t2);
                        int select = e.getStateChange();
                        ...
                        //if user diselects bed2(850), subtract the total payment and enable bed1(600)
                        else if(select == ItemEvent.DESELECTED)
                        {
                                price = price - 850.00 * (diffDays/7);
                                totalPrice.setText("Total Price: $" + price + "0 /week");
                ...
                //if user selects boat (60, optionally) calculate the boat payment separately and
                ...
                if(source == boat){
             	   diffDays1 = CalculationFrame.getDays1(t1a, t2a);
                   	...
                   	}//if user diselects boat, subtract the boat payment ONLY and update the total payment
                   	else if(select == ItemEvent.DESELECTED)
                       {
                               price = price - 60.00 * (diffDays1/7);
                               totalPrice.setText("Total Price: $" + price + "0 /week");
